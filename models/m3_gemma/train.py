...
def train():
    ...
    peft_config = LoraConfig(
        ...
        bias="none",
        task_type="CAUSAL_LM",
        target_modules=["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"] # Gemma targets
    )
    ...
    output_dir = os.path.join(config.MODELS_DIR, "m3_gemma/checkpoints")
    ...
    # CSV Log path
    csv_log_path = os.path.join(config.MODELS_DIR, "m3_gemma/logs/training_log.csv")
    ...
    print("Starting training M3 Gemma...")
    trainer.train()

    # Save final model
    final_output_dir = os.path.join(config.MODELS_DIR, "m3_gemma/final")
